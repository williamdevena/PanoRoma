...
class info_col : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val button = findViewById<Button>(R.id.colosse_map)
        button.setOnClickListener{
            val intent = Intent(
                    Intent.ACTION_VIEW,
                    Uri.parse("http://maps.google.com/maps?daddr=41.9135245551798, 12.477636485644826")
            )
            startActivity(intent)
        }

        val leggi = findViewById<TextView>(R.id.leggi)
        leggi.setOnClickListener{
            val myIntent = Intent(this, long_info_col::class.java)
